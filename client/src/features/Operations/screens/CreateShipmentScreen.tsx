...
export function CreateShipmentScreen() {
  ...
  const ordersForCreateShipment = orders.filter((o) => o.status === "ready_to_ship" && !o.shipmentId);
  const filteredOrders = useMemo(
    () => filterAndSortOrders(ordersForCreateShipment, searchQuery, dateFilter),
    [ordersForCreateShipment, searchQuery, dateFilter]
  );
    ...
    <>
      <Box sx={{ mb: 4 }}>
        ...
        <Typography sx={{ mt: 0.5, color: "text.secondary", fontSize: 14 }}>
          Orders ready to ship — enter carrier and tracking number to create shipment.
        </Typography>
      </Box>

      <Grid container spacing={2} sx={{ mb: 3 }}>
        <Grid item xs={12} sm={4}>
          <SummaryCard label="Orders awaiting shipment" value={ordersLoading ? "—" : ordersForCreateShipment.length} />
        </Grid>
      </Grid>
      ...
      <Paper elevation={0} sx={{ p: 3, borderRadius: 3, border: "1px solid rgba(0,0,0,0.08)" }}>
        ...
        {ordersLoading ? (
          ...
          <Typography color="text.secondary">No orders awaiting shipment.</Typography>
        ) : (
