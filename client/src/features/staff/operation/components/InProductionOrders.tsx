...
import { useGetInProductionOrders, useUpdateOrderStatus } from "../../../../lib/orderApi";
import OperationOrderDetailDialog from "./OperationOrderDetailDialog";

export default function InProductionOrders() {
  ...
  const { data: ordersData, isLoading } = useGetInProductionOrders(pageNumber, 10, {
  ...
  const statusColorMap: Record<string, "success" | "warning" | "info" | "primary"> = {
    ...
    <Box sx={{ p: 3, display: "flex", flexDirection: "column", gap: 2 }}>
      ...
      <TableContainer component={Card}>
        ...
        {ordersData?.items.length === 0 && !isLoading && (
          <Box sx={{ p: 2, textAlign: "center" }}>No orders in production</Box>
        )}
      </TableContainer>
      ...
      <Dialog open={completeDialogOpen} onClose={() => setCompleteDialogOpen(false)}>
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => orderToComplete && handleCompleteOrder(orderToComplete)}
            variant="contained"
            color="success"
            disabled={updateStatusMutation.isPending}
          >
